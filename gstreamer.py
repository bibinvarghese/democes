...
def run_pipeline(user_function,
    ...
    if detectCoralDevBoard():
        if jpginput: # This is faster for webcams that support it
          SRC_CAPS = 'image/jpeg,width={width},height={height} ! decodebin ! videoconvert  '
        else:
        ...
        if jpginput: # This is faster for webcams that support it
          SRC_CAPS = 'image/jpeg,width={width},height={height} ! jpegdec  '
        else:
          SRC_CAPS = 'video/x-raw,width={width},height={height},framerate=30/1'

        if not videosrc.startswith('/dev/video'):
          SRC_CAPS = 'video/x-raw,format=RGB,width={width},height={height},framerate=30/1'
        ...
        if use_appsrc:
            ...
            if videotgt == 'autovideosink':
              APPSRC_PIPELINE += '! autovideosink'
            else:
              APPSRC_PIPELINE += """! video/x-raw,format=I420,width=1280,height=720,framerate=30/1
                ! {leaky_q} ! {videoflip} ! videoconvert ! queue
                ! mpeg2enc bufsize=2000 bitrate=6400 ! filesink location=%s """%videotgt
            ...
            if videotgt == 'autovideosink':
              ...
              APPSRC_PIPELINE += ' ! qtmux ! filesink location=video.mov '
