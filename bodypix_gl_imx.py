...
from OpenGL.GLES3 import (
    ...
    GL_ONE_MINUS_SRC_ALPHA, GL_R16F, GL_R32F, GL_RED, GL_RGB, GL_SRC_ALPHA, GL_STATIC_DRAW, GL_TEXTURE0,
...
FRAGMENT_SHADER_SRC = '''
    ...
    uniform sampler2D image_tex, hm_tex, bg_tex, agg_hm_tex;
    uniform int stage;
    uniform float ratio;
    ...
    {
        float value = texture2D(hm_tex, v_texcoord).r;
        float a = v0 / (v0 - v1);
        float b = 1.0 / (v1 - v0);
    ...
    vec4 stage2_overlays()
    {
        ...
        vec4 body_outline = vec4(1.0,-0.2,-0.2, 0.7*heatmap);

        // Render density map using color hsv colormap but attenuate to
        // transparent at low density
        float agg_value = texture2D(agg_hm_tex, v_texcoord).r;
        vec4 rainbow = vec4(hsv2rgb(vec3(0.9-agg_value,
                                         1.0,
                                         clamp(agg_value*10.0, 0.0,1.0)
                                        )).xyz, clamp(agg_value*10.0, 0.0, 0.6)
                                    );

        return body_outline + rainbow;
    }
...
SVG_STYLES     = '''
    <style>
        ...
        .text_bg {{ fill: black; }}
        .text_fg {{ fill: white; }}
        .bbox {{ stroke: red; stroke-width: 2; fill: none;}}
...
class Callback:
    ...
    def init_gl(self, glcontext):
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, GL_R16F, hm_w, hm_h, 0, GL_RED, GL_FLOAT, None)

        self.agg_hm_tex_id = texture_ids[1]
    ...
    def update_heatmap(self, glcontext, hm_data):
        ...
        glBindTexture(GL_TEXTURE_2D, self.hm_tex_id)
        glPixelStorei(GL_UNPACK_ALIGNMENT, 1)
        glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, hm_w, hm_h, GL_RED, GL_FLOAT, hm_data)
    ...
    def generate_svg(self, poses, inference_box, text, people_count_log):
        ...
        if people_count_log: 
          p_last = None
          for i,o in enumerate(people_count_log):
            p_next = (self.vinfo.width-3*(len(people_count_log)-i), 40-o*2)
            if p_last is not None:
              svg.write(SVG_WHITE_LINE.format(x1=p_last[0], y1=p_last[1], 
    ...
    def run_inference(self, inf_buf, inf_caps):
        ...
        # Also have Callback query GPIO state
    ...
    def aggregate_buffers(self, inf_buf, inf_caps, vid_buf, vid_caps, box):
        ...
        start = time.monotonic()
        data = self.inf_q.get()
        poses, heatmap = self.engine.ParseOutputs(data)

        # Aggregate the heatmap data into our timeaveraged density map
        # using crude approx to sigmoid 1/(1 + np.exp(-heatmap))
        heatmap_prob = np.clip( 0.5 + 0.25 * heatmap, 0.0, 1.0);
        if self.heatmap_sum is None: self.heatmap_sum = heatmap_prob
        else: self.heatmap_sum += heatmap_prob

        # Upload the new heatmaps (instant and aggregated) texture data.
        self.glcontext.thread_add(self.update_aggregate_heatmap)
        self.glcontext.thread_add(self.update_heatmap, heatmap)
        ...
        elapsed = time.monotonic() - start
        self.agg_times.append(elapsed)
       
        # Average people estimate over last 60 frame (~2 seconds)
        self.people_count_last_n.append(len(poses))
        people_estimate = int(sum(self.people_count_last_n)/float(len(self.people_count_last_n)))
       
        # Track estimate every 60 seconds
        if self.frames % (60*25) == 0:
          self.people_count_log.append(len(poses))

        inf_time = 1000 * sum(self.inf_times) / len(self.inf_times)
        agg_time = 1000 * sum(self.agg_times) / len(self.agg_times)
        if self.print_stats and (self.frames % 100) == 0:
            print('Inference: {:.2f} ms ({:.2f} fps) Aggregate: {:.2f} ms ({:.2f} fps)'.format(
                    inf_time, 1000 / inf_time, agg_time, 1000 / agg_time))
        with self.trash_lock:
            self.trash_buffer(vid_buf)
            vid_buf = None
        text = 'Inference: {:.2f} ms ({:.2f} fps) Current occupancy: {:d}'.format(
                    inf_time, 1000 / inf_time,
                    people_estimate) 
        
        # Generate SVG overlay.
        svg = self.generate_svg(poses, box, text, self.people_count_log)
    ...
    def handle_stdin_char(self, char):
        if char == TOGGLE_SKELETONS:
            ...
            self.hm = not self.hm
        elif char == TOGGLE_AHM:
            self.ahm = not self.ahm
        elif char == TOGGLE_RESET:
          self.heatmap_sum = None
          self.frames = 1

    def update_button_LED_state(self):
        if not self.ui: return
        self.ui.setLED(0, False)
        self.ui.setLED(1, self.anon)
        self.ui.setLED(2, self.skeletons)
        self.ui.setLED(3, self.hm)
        self.ui.setLED(4, self.ahm)

    def read_gpio_state(self):
        if not self.ui: return
        button_state = self.ui.getDebouncedButtonState()

        ### Detect Hard reset (all 4 buttons pressed)
        if sum((1 for b in button_state if b)) >= 4:
          self.heatmap_sum = None
          self.frames = 0
          self.ui.wiggleLEDs(2)
          self.reset_display_toggles()
          self.frames = 1
        else:
          if button_state[0]:
          ...
          if button_state[2]:
            self.skeletons = not self.skeletons
            self.bboxes = self.skeletons
          if button_state[3]:
            self.hm = not self.hm
          if button_state[4]:
            self.ahm = not self.ahm
        self.update_button_LED_state()
...
def run_pipeline(cb, src_size, inference_size, video_src,
            h264=False, jpg=False, mirror=False):
...
def main():
    ...
    parser.add_argument('--jpg', help='Use video/x-h264 input', action='store_true')
    parser.add_argument('--print-stats', help='print timing stats', action='store_true')
    ...
    print(' TOGGLE_HM = h')
    print(' TOGGLE_AHM = g')
    run_pipeline(Callback(engine, save_frames=False, print_stats=args.print_stats), src_size,
            inference_size, video_src=args.videosrc, h264=args.h264, jpg=args.jpg,
            mirror=args.mirror)
